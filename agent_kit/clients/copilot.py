...
def _save_oauth_token_to_disk(oauth_token: str, user: str = "agent-kit") -> None:
...
class CopilotClient(ModelClient):
    ...
    Auth flow:
      ...
      2. If missing, raise — user must run `agent-kit copilot login` to start the device flow.
    ...
    def __init__(
        ...
        if not self._oauth_token:
            raise RuntimeError(
                "No Copilot OAuth token found. Run: `agent-kit copilot login` "
